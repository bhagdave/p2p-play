...
cd test-wasm-add && cargo build --target wasm32-wasip1 --release --quiet
if [ $? -ne 0 ]; then
...
run_test_suite "Running Network Protocol Integration Tests..." "cargo test --test network_protocol_integration_tests" "1"
run_test_suite "Running Multi-Peer Integration Tests..." "cargo test --test multi_peer_integration_tests" "1"
...
run_test_suite "Running Network Reconnection Tests..." "cargo test --test network_reconnection_tests" "1"
