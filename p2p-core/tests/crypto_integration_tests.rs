use libp2p::identity;
use p2p_core::crypto::{CryptoError, CryptoService};
...
async fn test_crypto_key_management() {
    ...
    let peer_keypair = identity::Keypair::generate_ed25519();
    let peer_public_key = peer_keypair.public().encode_protobuf();

    // Add peer key
    crypto_service
        .add_peer_public_key(peer_id, peer_public_key.clone())
        .expect("Adding peer key should succeed");

    // Test encryption works after adding key
    let message = b"test message";
    let result = crypto_service.encrypt_message(message, &peer_id);
    assert!(result.is_ok(), "Encryption should work after adding peer key");

    // Test that we can add multiple keys
    let peer_id2 = libp2p::PeerId::random();
    let peer_keypair2 = identity::Keypair::generate_ed25519();
    let peer_public_key2 = peer_keypair2.public().encode_protobuf();

    crypto_service
        .add_peer_public_key(peer_id2, peer_public_key2)
        .expect("Adding second peer key should succeed");

    let result2 = crypto_service.encrypt_message(message, &peer_id2);
    assert!(result2.is_ok(), "Encryption should work for second peer too");
}

#[tokio::test]
async fn test_crypto_large_message() {
    let alice_keypair = identity::Keypair::generate_ed25519();
    let bob_keypair = identity::Keypair::generate_ed25519();
    ...
    let encrypted = alice_crypto
        .encrypt_message(&large_message, &bob_peer_id)
