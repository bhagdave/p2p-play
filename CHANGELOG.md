...
- **Removed ability to set fule and memory limits via `wasm run` arguments**: Resource limits for remote WASM execution are now only configurable via the unified network configuration file. This simplifies the command interface and ensures consistent application of limits across all executions. The `wasm run` command now uses the configured defaults for fuel, memory, and timeout without accepting overrides as command-line arguments.
...
- **WASM offering**: Offering ID is now shown in both `wasm ls remote` and the `wasm query <peer>` response, making it straightforward to copy the ID directly into a `wasm run` command

### Fixed
- **Saved peer name lost after running tests**: Two tests in `integration_tests.rs` (`test_stories_with_channels` and `test_channel_subscription_data_structures`) did not set up an isolated test database. When run after other tests that removed the `TEST_DATABASE_PATH` environment variable, these tests would fall back to `./stories.db` (the production database) and clear it via `clear_database_for_testing()`, erasing the saved peer name. Fixed by giving each test its own isolated database path and removing the unnecessary database calls from the pure-type-serialisation test.
- **WASM Execution**: Fixed the WASI context to prepend a synthetic `argv[0]` ("wasm_module") automatically. Callers should pass only the actual arguments after `argv[0]`; the program name is no longer required in `ExecutionRequest.args`.
...
### Added
- **Story export (`export s`)**: Export local stories to the `./exports/` directory in Markdown or JSON format.
  - `export s <id> md` — single story as a Markdown file with YAML front-matter (id, channel, public, created_at)
  - `export s <id> json` — single story as a JSON object file
  - `export s all md` — all local stories as individual `.md` files
  - `export s all json` — all local stories as a JSON array in `stories.json`
  - The `exports/` directory is created automatically on first use and is excluded from version control via `.gitignore`.
...
- **WASM Configuration Support**: Added WASM executor resource limits to the unified network configuration
  ...
  - **Runtime Validation**: WASM executor validates memory limits against configured maximum at execution time
