...
pub async fn write_local_stories_to_path(stories: &Stories, path: &str) -> Result<(), Box<dyn Error>> {
    let json = serde_json::to_string(&stories)?;
...
pub async fn create_new_story_in_path(name: &str, header: &str, body: &str, path: &str) -> Result<usize, Box<dyn Error>> {
...
pub async fn save_received_story_to_path(mut story: Story, path: &str) -> Result<usize, Box<dyn Error>> {
    let mut local_stories = match read_local_stories_from_path(path).await {
        Ok(stories) => stories,
        Err(_) => Vec::new(),
    };
    ...
    if !already_exists {
        ...
        let existing = local_stories.iter().find(|s| 
            s.name == story.name && s.header == story.header && s.body == story.body
        ).unwrap();
        Ok(existing.id)
    }
...
mod tests {
    ...
    async fn create_temp_stories_file() -> NamedTempFile {
        ...
        write_local_stories_to_path(&initial_stories, temp_file.path().to_str().unwrap()).await.unwrap();
        temp_file
    }
    ...
    async fn test_create_new_story() {
        ...
        let new_id = create_new_story_in_path("New Story", "New Header", "New Body", path).await.unwrap();
    ...
    async fn test_create_story_in_empty_file() {
        ...
        let new_id = create_new_story_in_path("First Story", "Header", "Body", path).await.unwrap();
        let stories = read_local_stories_from_path(path).await.unwrap();
    ...
    async fn test_publish_story() {
        ...
        let story_id = create_new_story_in_path("Test Publish", "Header", "Body", path).await.unwrap();
    ...
    async fn test_save_received_story() {
        ...
        let new_id = save_received_story_to_path(received_story, path).await.unwrap();
        let stories = read_local_stories_from_path(path).await.unwrap();
    ...
    async fn test_save_duplicate_received_story() {
        ...
        // Save first time
        let first_id = save_received_story_to_path(story.clone(), path).await.unwrap();
    ...
    async fn test_story_id_sequencing() {
        ...
        let id1 = create_new_story_in_path("Story 1", "H1", "B1", path).await.unwrap();
        let id2 = create_new_story_in_path("Story 2", "H2", "B2", path).await.unwrap();
        let id3 = create_new_story_in_path("Story 3", "H3", "B3", path).await.unwrap();
    ...
    async fn test_publish_with_channel() {
        ...
        let story_id = create_new_story_in_path("Channel Test", "Header", "Body", path).await.unwrap();
        let published = publish_story_in_path(story_id, path).await.unwrap().unwrap();
