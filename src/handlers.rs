...
pub async fn handle_help(_cmd: &str, ui_logger: &UILogger) {
    ...
    ui_logger.log("  Example: search rust channel:tech recent:7 public".to_string());
...
/// Handle WASM commands
pub async fn handle_wasm_command(
    cmd: &str,
    swarm: &mut Swarm<StoryBehaviour>,
    peer_names: &std::collections::HashMap<PeerId, String>,
    local_peer_name: &Option<String>,
    ui_logger: &UILogger,
    error_logger: &ErrorLogger,
) {
...
async fn handle_wasm_run(
    ...
    let request = crate::network::WasmExecutionRequest {
        from_peer_id: PEER_ID.to_string(),
        from_name,
        offering_id: offering_id.to_string(),
        ipfs_cid,
        input: Vec::new(), // Could be extended to read from stdin or file
        args: run_args,
        fuel_limit: None,
