...
fn detect_protocol_from_error(error_message: &str) -> String {
    ...
    if lower_msg.contains("floodsub") || lower_msg.contains("pubsub") || lower_msg.contains("topic")
    {
        ...
        "ping".to_string()
    } else if lower_msg.contains("kad")
        || lower_msg.contains("kademlia")
        || lower_msg.contains("dht")
    {
        "kad".to_string()
    } else if lower_msg.contains("request_response") || lower_msg.contains("request-response") {
        "request_response".to_string()
    } else if lower_msg.contains("transport")
        || lower_msg.contains("tcp")
        || lower_msg.contains("quic")
    {
        "transport".to_string()
...
mod tests {
    ...
    fn test_protocol_detection() {
        ...
        assert_eq!(
            detect_protocol_from_error("floodsub connection failed"),
            "floodsub"
        );
        assert_eq!(
            detect_protocol_from_error("topic subscription error"),
            "floodsub"
        );
        ...
        assert_eq!(
            detect_protocol_from_error("tcp connection refused"),
            "transport"
        );
        assert_eq!(
            detect_protocol_from_error("quic handshake failed"),
            "transport"
        );
        ...
        // Test unknown
        assert_eq!(
            detect_protocol_from_error("generic error message"),
            "unknown"
        );
    }
    ...
    fn test_batch_operation_error() {
        let failures = vec![
            "Story 1 not found".to_string(),
            "Story 3 invalid".to_string(),
        ];
