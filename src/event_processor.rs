...
use crate::daemon::protocol::{
    ...
    MessagesSummary, PeerInfo, StoryDetail, StoryInfo,
};
...
impl EventProcessor {
    ...
    async fn dispatch_daemon_command(
        ...
        match req {
            ...
            DaemonRequest::GetStory { id } => match storage::read_story_by_id(id).await {
                Ok(Some(story)) => {
                    let _ = txt.send(DaemonResponse::Story {
                        story: StoryDetail {
                            id: story.id,
                            name: story.name,
                            header: story.header,
                ...
                Err(e) => {
                    let _ = txt.send(DaemonResponse::Error {
                        message: format!("Failed to load story: {e}"),
                    });
                }
            },
