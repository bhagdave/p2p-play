...
use crate::network::{PEER_ID, StoryBehaviour, StoryBehaviourEvent, TOPIC, HandshakeRequest, APP_NAME, APP_VERSION};
...
impl EventProcessor {
    ...
    async fn handle_connection_established(
        ...
        debug!("Initiating handshake with newly connected peer: {}", peer_id);
        ...
        let request_id = swarm.behaviour_mut().handshake.send_request(&peer_id, handshake_request);
