...
impl NetworkConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.connection_maintenance_interval_seconds < 30 {
            return Err("connection_maintenance_interval_seconds must be at least 30 seconds to avoid excessive connection churn".to_string());
        }
