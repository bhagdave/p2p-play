...
async fn test_story_workflow_integration() {
    ...
    let received_id = save_received_story_to_path(received_story, path2).await.unwrap();
...
async fn test_multiple_peers_story_sharing() {
    ...
    let story1_id = create_new_story_in_path("Peer 1 Story", "Header 1", "Body 1", peer1_path).await.unwrap();
    let story2_id = create_new_story_in_path("Peer 2 Story", "Header 2", "Body 2", peer2_path).await.unwrap();
    let story3_id = create_new_story_in_path("Peer 3 Story", "Header 3", "Body 3", peer3_path).await.unwrap();
    
    // Each peer publishes their story
    let published1 = publish_story_in_path(story1_id, peer1_path).await.unwrap().unwrap();
    let published2 = publish_story_in_path(story2_id, peer2_path).await.unwrap().unwrap();
    let published3 = publish_story_in_path(story3_id, peer3_path).await.unwrap().unwrap();
    
    // Simulate story distribution - each peer receives stories from others
    save_received_story_to_path(published2.clone(), peer1_path).await.unwrap();
    save_received_story_to_path(published3.clone(), peer1_path).await.unwrap();
    
    save_received_story_to_path(published1.clone(), peer2_path).await.unwrap();
    save_received_story_to_path(published3.clone(), peer2_path).await.unwrap();
    
    save_received_story_to_path(published1.clone(), peer3_path).await.unwrap();
    save_received_story_to_path(published2.clone(), peer3_path).await.unwrap();
...
async fn test_story_deduplication() {
    ...
    let id1 = save_received_story_to_path(story.clone(), path).await.unwrap();
    let id2 = save_received_story_to_path(story.clone(), path).await.unwrap();
    let id3 = save_received_story_to_path(story.clone(), path).await.unwrap();
...
async fn test_list_request_response_cycle() {
    ...
    let story1 = Story::new(1, "Public Story".to_string(), "Header".to_string(), "Body".to_string(), true);
    let story2 = Story::new(2, "Private Story".to_string(), "Header".to_string(), "Body".to_string(), false);
...
async fn test_published_story_message() {
    let story = Story::new(1, "Published Test".to_string(), "Header".to_string(), "Body".to_string(), true);
...
async fn test_error_handling() {
    ...
    write_local_stories_to_path(&Vec::new(), path).await.unwrap();
...
async fn test_story_filtering() {
    ...
    create_new_story_in_path("Private 1", "H1", "B1", path).await.unwrap();
    let public_id = create_new_story_in_path("Public 1", "H2", "B2", path).await.unwrap();
    create_new_story_in_path("Private 2", "H3", "B3", path).await.unwrap();
