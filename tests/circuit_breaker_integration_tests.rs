...
async fn test_circuit_breaker_integration_with_network_operations() {
    ...
    let result = network_breakers.execute("story_publish", || async {
...
async fn test_circuit_breaker_recovery_after_timeout() {
    ...
    let result = network_breakers.execute("peer_connection", || async {
...
async fn test_disabled_circuit_breakers() {
    ...
    for _ in 0..10 {
        let result = network_breakers.execute("test_operation", || async {
...
async fn test_network_health_summary_functionality() {
    ...
    assert!(status.contains(&format!("{}/{}", health.total_operations, health.total_operations)));
    
    // Cause some failures
    network_breakers.on_failure("message_broadcast", "Test failure").await;
...
async fn test_circuit_breaker_timeout_handling() {
    ...
    let result = network_breakers.execute("dht_bootstrap", || async {
    ...
    assert!(result.is_err());
    if let Err(CircuitBreakerError::OperationTimeout { circuit_name, timeout }) = result {
