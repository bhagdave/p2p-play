...
async fn test_reconnection_speed_measurement() {
    ...
    let unified_config = UnifiedNetworkConfig::load_from_file("unified_network_config.json").unwrap();
    assert_eq!(unified_config.network.connection_maintenance_interval_seconds, 10);
    ...
    assert_eq!(unified_config.network.connection_establishment_timeout_seconds, 45);
...
async fn test_network_stability_with_optimizations() {
    ...
    assert_eq!(loaded_config.network.connection_maintenance_interval_seconds, 10);
    assert_eq!(loaded_config.network.connection_establishment_timeout_seconds, 45);
...
fn cleanup_old_throttling_entries(
    ...
    let now = Instant::now();
    peer_throttling.retain(|_, timestamp| {
...
fn cleanup_old_successful_connections(
    ...
    let now = Instant::now();
    successful_connections.retain(|_, timestamp| {
        now.duration_since(*timestamp) < max_age
    });
}