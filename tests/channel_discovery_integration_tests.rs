...
use p2p_play::storage::{
    create_channel, get_channels_for_stories, process_discovered_channels, 
    read_channels, save_received_story
};
...
async fn test_channel_discovery_integration_workflow() {
    ...
    create_channel("technology", "Technology discussions", "nodeA").await.unwrap();
    ...
    println!("4. Channel metadata found: {:?}", 
    ...
    println!("6. Node B initial channels: {:?}", 
    ...
    let discovered_count = process_discovered_channels(&channel_metadata, "nodeA").await.unwrap();
    ...
    println!("11. Node B final channels: {:?}", 
    ...
    let tech_channel = final_channels.iter().find(|c| c.name == "technology").unwrap();
...
async fn test_channel_discovery_with_missing_metadata() {
    ...
    println!("2. Channel metadata created: {:?}", 
        channel_metadata.iter().map(|c| (&c.name, &c.description)).collect::<Vec<_>>());
    ...
    let discovered_count = process_discovered_channels(&channel_metadata, "unknown_peer").await.unwrap();
    assert_eq!(discovered_count, 1);
...
async fn test_backward_compatibility() {
    ...
    let discovered_count = process_discovered_channels(&empty_channels, "old_node").await.unwrap();
