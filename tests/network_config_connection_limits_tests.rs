...
async fn test_network_config_load_and_save_with_connection_limits() {
    ...
    // Save the config
    original_config
        .save_to_file(config_path_str)
        .expect("Failed to save config");

    // Load the config back
    let loaded_config =
        NetworkConfig::load_from_file(config_path_str).expect("Failed to load config");
...
async fn test_network_config_serialization_format() {
    ...
    assert_eq!(
        deserialized.max_connections_per_peer,
        config.max_connections_per_peer
    );
    assert_eq!(
        deserialized.max_pending_incoming,
        config.max_pending_incoming
    );
    assert_eq!(
        deserialized.max_pending_outgoing,
        config.max_pending_outgoing
    );
    assert_eq!(
        deserialized.max_established_total,
        config.max_established_total
    );
    assert_eq!(
        deserialized.connection_establishment_timeout_seconds,
        config.connection_establishment_timeout_seconds
    );
}
