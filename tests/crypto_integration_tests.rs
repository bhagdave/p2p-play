...
async fn test_crypto_integration_with_libp2p_keys() {
    ...
    let encrypted = alice_crypto.encrypt_message(original_message, &bob_peer_id)
    ...
    let encrypted_reply = bob_crypto.encrypt_message(bob_message, &alice_peer_id)
        .expect("Encryption should succeed");
    
    let decrypted_reply = alice_crypto.decrypt_message(&encrypted_reply)
        .expect("Decryption should succeed");
...
async fn test_crypto_signature_integration() {
    ...
    let is_tampered_valid = crypto_service.verify_signature(tampered_message, &signature)
        .expect("Verification should succeed");
