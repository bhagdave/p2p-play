...
async fn test_auto_subscription_config() {
    ...
    // Test default values
    assert!(!config.auto_subscribe_to_new_channels);
    assert!(config.notify_new_channels);
...
async fn test_unified_config_with_auto_subscription() {
    ...
    assert!(config.channel_auto_subscription.notify_new_channels);
...
async fn test_available_vs_subscribed_channels() {
    ...
    create_channel("rust-programming", "Rust programming discussions", "alice")
        .await
        .expect("Failed to create channel");
    create_channel("testing", "Test discussions", "system")
        .await
        .expect("Failed to create channel");
    create_channel("announcements", "Important announcements", "bob")
        .await
        .expect("Failed to create channel");

    // Test reading all available channels (3 created, default "general" gets cleared in test init)
    let available = read_channels()
        .await
        .expect("Failed to read available channels");
    assert_eq!(available.len(), 3);

    // Test reading unsubscribed channels (should be all channels initially)
    let unsubscribed = read_unsubscribed_channels(peer_id)
        .await
        .expect("Failed to read unsubscribed channels");
    assert_eq!(unsubscribed.len(), 3);

    // Subscribe to one channel
    subscribe_to_channel(peer_id, "rust-programming")
        .await
        .expect("Failed to subscribe");

    // Test unsubscribed channels after subscription
    let unsubscribed_after = read_unsubscribed_channels(peer_id)
        .await
        .expect("Failed to read unsubscribed channels");
    ...
    assert!(
        !unsubscribed_after
            .iter()
            .any(|ch| ch.name == "rust-programming")
    );

    // Test subscription count
    let sub_count = get_auto_subscription_count(peer_id)
        .await
        .expect("Failed to get subscription count");
    assert_eq!(sub_count, 1);

    // Subscribe to another channel
    subscribe_to_channel(peer_id, "testing")
        .await
        .expect("Failed to subscribe");

    // Test updated counts
    let sub_count_after = get_auto_subscription_count(peer_id)
        .await
        .expect("Failed to get subscription count");
    assert_eq!(sub_count_after, 2);

    let unsubscribed_final = read_unsubscribed_channels(peer_id)
        .await
        .expect("Failed to read unsubscribed channels");
    assert_eq!(unsubscribed_final.len(), 1);

    // Should have "announcements" unsubscribed
    let unsubscribed_names: Vec<&str> = unsubscribed_final
        .iter()
...
async fn test_channel_auto_subscription_config_persistence() {
    ...
    // Save config
    save_unified_network_config_to_path(&config, config_path.to_str().unwrap())
        .await
        .expect("Failed to save config");

    // Load config
    let loaded_config = load_unified_network_config_from_path(config_path.to_str().unwrap())
        .await
...
async fn test_auto_subscription_logic() {
    ...
    create_channel("auto-test-channel", "Auto subscription test", "creator")
        .await
        .expect("Failed to create channel");
    ...
    // Test subscription count
    let count = get_auto_subscription_count(peer_id)
        .await
    ...
    for i in 1..=12 {
        ...
        create_channel(&channel_name, "Test channel", "creator")
            .await
            .expect("Failed to create channel");
        subscribe_to_channel(peer_id, &channel_name)
            .await
            .expect("Failed to subscribe");
    }

    // Should have 13 subscriptions now (1 + 12)
    let final_count = get_auto_subscription_count(peer_id)
        .await
        .expect("Failed to get count");
    assert_eq!(final_count, 13);

    // Test unsubscribed channels
    create_channel("unsubscribed-channel", "Not subscribed to this", "creator")
        .await
        .expect("Failed to create channel");
    let unsubscribed = read_unsubscribed_channels(peer_id)
        .await
        .expect("Failed to read unsubscribed");
    assert!(
        unsubscribed
            .iter()
            .any(|ch| ch.name == "unsubscribed-channel")
    );
