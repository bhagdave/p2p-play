use libp2p::{
    PeerId, StreamProtocol, Transport, core::upgrade, dns, noise, request_response,
    swarm::Config as SwarmConfig, tcp, yamux,
};
...
pub fn create_test_swarm_with_keypair(
    ...
    let story_sync_protocols =
        iter::once((story_sync_protocol, request_response::ProtocolSupport::Full));
    ...
    Ok(libp2p::Swarm::new(transp, behaviour, peer_id, swarm_config))
}

/// Creates multiple test swarms with unique peer IDs
pub async fn create_test_swarms(
    count: usize,
) -> Result<Vec<libp2p::Swarm<StoryBehaviour>>, Box<dyn std::error::Error>> {
