...
use p2p_play::relay::{RelayService, RelayAction};
use p2p_play::types::{RelayConfig, DirectMessage};
...
async fn test_relay_message_creation_and_processing() {
    ...
    alice_crypto.add_peer_public_key(bob_peer_id, bob_public_key).expect("Alice should add Bob's key");
    bob_crypto.add_peer_public_key(alice_peer_id, alice_public_key).expect("Bob should add Alice's key");
    ...
    let relay_message = alice_relay.create_relay_message(&direct_message, &bob_peer_id)
    ...
    let result = bob_relay.process_relay_message(&relay_message)
    ...
    match result {
        RelayAction::DeliverLocally(decrypted_msg) => {
            ...
            assert_eq!(decrypted_msg.message, "Hello Bob! This is a secure relay message.");
...
async fn test_relay_message_forwarding() {
    ...
    alice_relay.crypto_service().add_peer_public_key(charlie_peer_id, charlie_public_key)
    ...
    let relay_message = alice_relay.create_relay_message(&direct_message, &charlie_peer_id)
        .expect("Alice should create relay message");
    ...
    match result {
        ...
        RelayAction::ForwardMessage(forwarded_msg) => {
            ...
            let charlie_result = charlie_relay.process_relay_message(&forwarded_msg)
...
async fn test_relay_max_hops_limit() {
    ...
    let mut relay_message = p2p_play::types::RelayMessage {
    ...
    let result = relay_service.process_relay_message(&relay_message)
...
async fn test_relay_rate_limiting() {
    ...
    // Exchange keys
    let bob_public_key = bob_keypair.public().encode_protobuf();
    alice_crypto.add_peer_public_key(bob_peer_id, bob_public_key).expect("Should add Bob's key");
