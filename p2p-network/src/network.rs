...
use crate::types::{NetworkConfig, NetworkEvent, PeerInfo, Message, MessageContent};
...
// Re-export key libp2p types for convenience
pub use libp2p::{identity, noise, yamux, tcp, dns};
...
impl P2PNetwork {
    ...
    async fn handle_request_response_event(&mut self, event: libp2p::request_response::Event<Message, Message>) -> Option<NetworkEvent> {
        ...
        match event {
            Event::Message { peer, message } => {
                match message {
                    libp2p::request_response::Message::Request { request, .. } => {
                        ...
                        let final_message = if let Some(crypto) = &self.crypto_service {
                            match crypto.decrypt_message(&request, &peer).await {
                                Ok(decrypted) => decrypted,
                                Err(e) => {
                                    log::warn!("Failed to decrypt message from {}: {}", peer, e);
                                    return None;
                                }
                            }
                        } else {
                        ...
                        Some(NetworkEvent::DirectMessageReceived { 
                            from: peer, 
            ...
            Event::OutboundFailure { peer, error, .. } => {
                Some(NetworkEvent::NetworkError { 
                    error: format!("Outbound request to {} failed: {:?}", peer, error) 
                })
            }
            Event::InboundFailure { peer, error, .. } => {
                Some(NetworkEvent::NetworkError { 
                    error: format!("Inbound request from {} failed: {:?}", peer, error) 
                })
            }
            _ => None,
        }
...
impl From<libp2p::mdns::Event> for NetworkBehaviourEvent {
...
impl From<libp2p::request_response::Event<Message, Message>> for NetworkBehaviourEvent {
    fn from(event: libp2p::request_response::Event<Message, Message>) -> Self {
        NetworkBehaviourEvent::RequestResponse(event)
    }
}

impl From<libp2p::kad::Event> for NetworkBehaviourEvent {
