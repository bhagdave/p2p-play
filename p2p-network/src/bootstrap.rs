...
pub struct AutoBootstrap {
    ...
    status: Arc<Mutex<BootstrapStatus>>,
...
impl AutoBootstrap {
    ...
    pub fn add_peer(&mut self, address: Multiaddr) {
        self.bootstrap_peers.push(address);
    }

    pub async fn run_bootstrap(&mut self, swarm: &mut Swarm<NetworkBehaviour>) -> BootstrapResult<()> {
