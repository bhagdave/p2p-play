...
async fn test_story_chronological_order_and_channel_display() {
    ...
    assert!(
        ...
        "Stories are not in chronological order (newest first): {} >= {}",
        stories[1].created_at,
        stories[2].created_at
    );
    println!("✅ Stories are correctly ordered chronologically (newest first)");

    // Verify newest story is "Third Story"
    assert_eq!(
        stories[0].name, "Third Story",
        "Newest story should be 'Third Story'"
    );
    assert_eq!(
        stories[1].name, "Second Story",
        "Second newest story should be 'Second Story'"
    );
    assert_eq!(
        stories[2].name, "First Story",
        "Oldest story should be 'First Story'"
    );

    // Verify channel info is preserved
    assert_eq!(
        stories[0].channel, "general",
        "Third story should be in 'general' channel"
    );
    assert_eq!(
        stories[1].channel, "tech",
        "Second story should be in 'tech' channel"
    );
    assert_eq!(
        stories[2].channel, "general",
        "First story should be in 'general' channel"
    );
    ...
    for story in &stories {
        assert!(
            story.created_at > 0,
            "Story '{}' should have a valid timestamp",
            story.name
        );
    }
    ...
    for story in &stories {
        ...
        assert!(
            ui_format.contains(&format!("[{}]", story.channel)),
