...
use p2p_play::types::{Conversation, DirectMessage};
...
async fn test_empty_conversations_list() {
    ...
    assert!(
        ...
        "Should return empty conversations list when no messages exist"
    );
...
async fn test_get_conversation_messages_empty() {
    ...
    assert!(
        messages.is_empty(),
        "Should return empty vector for nonexistent conversation"
    );
...
async fn test_mark_conversation_messages_as_read() {
    ...
    for i in 1..=3 {
        let msg = create_test_direct_message(
            &peer1_str,
            local_str,
            &format!("Message {}", i),
            1000 + i,
    ...
    save_direct_message(&outgoing, Some(&peer_names))
        .await
        .unwrap();
